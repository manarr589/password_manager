...
def open_new_window():
    ...
    file_path = "password_manger.py"

    try:
        with open(file_path, 'r') as file:
            content = file.read()

        label = tk.Label(new_window, text=content)
        label.pack(pady=20)
    except FileNotFoundError:
        label = tk.Label(new_window, text="File not found.")
        label.pack(pady=20)
...
def signin():
    ...
    if username in r.keys() and password ==r[username]:
        ...
        os.system('python password_manger.py')
...
heading = Label(frame, text="Sign in", fg="#57a1f8", bg='white', font=("Calibri(Body)", 23, "bold"))
heading.place(x=100, y=5)
...
Button(frame, width=39, pady=7, text="Sign in", bg="#57a1f8", fg="white", border=0, command=signin, relief="raised").place(x=35, y=204)
label = Label(frame, text="Don't have an account?", fg="black", bg="white", font=("Calibri(Body)", 9))
label.place(x=75, y=270)
signup_button = Button(frame, text="Sign up", bg="white", fg="#57a1f8", border=0, cursor='hand2', command=signup_command)
